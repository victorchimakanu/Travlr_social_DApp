...
class Post {
  constructor(id, title, description, tags, media) {
    ...
    this.owner_id = predecessorAccountId(); // refers to whoeevr called the function that triggered this constructor
  }
}
let TravlrGaming = (_dec = NearBindgen({}), _dec2 = call({}), _dec3 = view({}), _dec4 = call({}), _dec5 = call({}), _dec6 = view({}), _dec(_class = (_class2 = class TravlrGaming {
  // shows all the posts user has liked
  // shows all the posts by a gamer tag 
  ...
  //function is of type view because it doesnt chnag eany of the data in the contract (read only)
  get_all_posts({}) {
  ...
  like_a_post({
    ...
    var post = this.posts.get(postId); // gets the post we are going to like 
    var sender_id = predecessorAccountId(); //whoever called the function is sender id
  ...
  //because the function will not change any data and doesnt require a signer 
  get_posts_by_tag({
    tag
  }) {
    if (this.posts_by_tag.get(tag) == null) {
...
function get_posts_by_tag() {
  let _state = TravlrGaming._getState();
  if (!_state && TravlrGaming._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  let _contract = TravlrGaming._create();
  if (_state) {
    TravlrGaming._reconstruct(_contract, _state);
  }
  let _args = TravlrGaming._getArgs();
  let _result = _contract.get_posts_by_tag(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(TravlrGaming._serialize(_result));
}
function get_my_liked_posts() {
  ...
  TravlrGaming._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(TravlrGaming._serialize(_result));
}
